...
# Model: Ridge (L2)
# From last model, we took info that alpha closer to 0 was the best for RMSE
df_results = pd.DataFrame(data=[])
for a in np.linspace(start=0.001, stop=2, num=50):
...
y_pred, _ = regr_lasso(x_train, x_test, y_train, alpha=best_alpha)
results = regr_metrics(y_test, y_pred)

# Plots
plot_lineduo(x1=df_results.index, y1=df_results["mae"], label1="MAE",
             y2=df_results["rmse"], label2="RMSE", xlabel="alpha",
             title=f"Concrete Strength - Model - LinRegr Lasso - Detail", savefig=savefig)
