...
def clf_kneighbors(x_train, x_test, y_train, n_neighbors=2, weights="uniform"):
    ...
    * weights: Weight to use for error. Could be uniform or distance. (default=uniform)
    ...
    # Probability output
    predict_proba = pd.DataFrame(data=[])
    for i in x_test.index:
        value = np.array(x_test.loc[i, :])
        prob = clf.predict_proba([value])
        predict_proba.loc[i, "predicted"] = prob[0][1]
...
df_train = prepare_dataset(filename="pm_train.txt", path=path_database)
df_train = apply_sma(df_train, columns=cols_tags(), window=6)
df_train = balance_uniform(df_train)

df_test = prepare_dataset(filename="pm_test.txt", path=path_database)

target = "failure_flag"
# Production Model
x_train, y_train = target_split(df_train, target=target)
x_test, y_test = target_split(df_test, target=target)

cols_remove = ["asset_id", "runtime", "runtime_inv","setting_1", "setting_2"]
x_train = remove_cols_for_train(x_train, columns=cols_remove)
x_test = remove_cols_for_train(x_test, columns=cols_remove)

x_train, x_test = apply_scaler(x_train, x_test, scaler=StandardScaler())
y_pred, y_proba = clf_kneighbors(x_train, x_test, y_train, n_neighbors=19, weights="uniform")
results = clf_metrics(y_test, y_pred, show_matrix=True)

y_proba.to_csv("submissao_eduardo.csv", index=False, encoding="utf-8")
