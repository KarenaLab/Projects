...
for i, [train_index, test_index] in folds.items():
    ...
    # 3.1- Model: K Neighbors
    y_pred, _, y_params = clf_kneighbors(x_train, x_test, y_train, n_neighbors=19, weights="uniform")
    results = clf_metrics(y_test, y_pred, show_matrix=True)

    for j in ["tpr", "fpr", "fnr", "tnr"]:
        df_results.loc[1, f"fold_{i}_{j}"] = results[j]


# Final Model
x_train, y_train = target_split(df, target=target)
